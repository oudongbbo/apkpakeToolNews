...
public class BaseBuilder {
    ...
    public static int SDK = Platforms.SDK_LEIDONG;
    ...
    private void handleMajiaSDK() {
        ...
        if ("fanxun".equals(sdkCompany)) {
            ...
            String GamePath = assetsRootPath + File.separator + "leidongGame.ini";
            String MsdkPath = assetsRootPath + File.separator + "leidongMsdk.ini";
...
//            FileUtil.replaceDirAndFileName(new File(smaliRootPath), "maya", "fanxun");
            ...
            FileUtil.replaceDirAndFileName(new File(smaliRootPath), "leidongMsdk", "FxMsdk");
            ...
            //参数配置：leidongGame.ini   fanxunGame.ini
            //参数配置：leidongMsdk.ini   fanxunMsdk.ini
            ...
            contents.put("leidongGame.ini", "fanxunGame.ini");
            contents.put("leidongMsdk.ini", "fanxunMsdk.ini");
...
//        if (SDK == Platforms.SDK_LEIDONG && GUANFANG_SDK_TAG.equals(sdkCompany)) {
//
//            LogUtil.showLog("如果是母包，则删除leidongMsdk.ini文件");
//
...
//                        + "leidongMsdk.ini";
//            } else {
//                delcmd = " zip  -d " + resourceApkPath + " assets" + File.separator + "leidongMsdk.ini ";
//            }
//
...
//                if (proc.waitFor() != 0) {
//                    if (proc.exitValue() == 0)
//                        LogUtil.showLog("leidongMsdk.ini 删除失败");
//                } else {
//                    LogUtil.showLog("leidongMsdk.ini 删除成功");
//                }
//
//            } catch (Exception e) {
//                e.printStackTrace();
...
//        }
    ...
    private void beforeCopy() {
        ...
        handleGameConfig(); // 重写leidongGame.ini
    ...
    /**
     * 删除data下面的leidongGame.ini文件 重新apktemp下的leidongGame.ini文件
     */
    private void handleGameConfig() {

        // 1.删除data下面的leidongGame.ini文件
        File dataFile = new File(Common.getDatasPathBySDK(SDK) + System.getProperty("file.separator") + "assets"
                + System.getProperty("file.separator") + "leidongGame.ini");

        if (dataFile.exists()) {
            FileUtil.delDir(dataFile);
            LogUtil.showLog("删除data里的leidongGame.ini：" + dataFile.toString());
        }

        // 2.反编译后temp下面的leidongGame.ini文件
    ...
    /**
     * 处理apktemp下的leidongMsdk.ini文件 由第三方渠道集成，按需修改
     */
    public void handleMsdkConfig() {

        LogUtil.showLog("处理apktemp下的leidongMsdk.ini文件", LogLevel.LEVEL_1);

        String tempPath = Common.getApkTempPath(apkParser) + System.getProperty("file.separator") + "assets"
                + System.getProperty("file.separator") + "leidongMsdk.ini";
...
//        if (SDK == PlatformConfig.SDK_LEIDONG) {
